...
double speed_kmh = 70;
uint32_t intensityOfRain = 20; // rain intensity in mm/h, not worked
...
Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream ("pathloss_s70_i20.0_k1.0_a0.1_rain.txt");
...
int main(int argc, char *argv[]) {
  ...
  ns3::Time simulationTime(ns3::Seconds(190)); // according to SUMO time
  double stepTime = 1.0;
  CommandLine cmd;
...
void computePathLoss(NetDeviceContainer devs1, NetDeviceContainer devs2, double stepTime) {
  ...
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << speed_kmh << "\t" << intensityOfRain << "\t" << k << "\t" << alpha << "\t" << pathLossVal << "\t" << distance3D << "\t" << weatherAtten << "\t" << stepTime << std::endl;
